...
import { Supplier, Purchase, CurrencyCode } from '../types';
import { formatCurrency } from '../utils/calculations';
import { Truck, Plus, DollarSign, X, Check } from 'lucide-react';
...
export const SuppliersView: React.FC<SuppliersViewProps> = ({
  ...
  const totalOwedToSuppliers = suppliers.reduce((acc, s) => acc + (s.amountOwed || 0), 0);
    ...
    <div id="smartledger-suppliers-view" className="space-y-6">
        ...
        <div className="flex items-center gap-2">
          {onOpenPurchaseOrder && (
            ...
            >
              ...
              <span>Purchase Order & Receive</span>
            </button>
          )}
      ...
      <div className="p-4 bg-slate-100 border border-slate-200 rounded-2xl flex items-center justify-between">
        ...
        <div className="text-right text-xs text-slate-600">
          <span className="font-bold">{suppliers.filter((s) => s.amountOwed > 0).length}</span> suppliers pending payment
        </div>
      </div>
            ...
            >
              <div>
                <div className="flex items-start justify-between gap-2">
                  ...
                  >
                    {hasDebt ? 'You Owe Money' : 'Paid In Full'}
                  </span>
                </div>
            ...
            <form onSubmit={handleSavePayment} className="p-5 space-y-4">
              <div className="p-3 bg-slate-50 rounded-xl text-xs space-y-1">
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-amber-700">{formatCurrency(selectedSupplier.amountOwed, currency)}</span>
                </div>
              </div>
